...
function loadJSON() {
  fetch("products.json")
    ...
    .then((data) => {
      ...
      data.forEach((product) => {
        ...
        <div class="product-list p-2.5">
            <img class="product-img w-64 ml-auto mr-auto block max-w-44 max-h-60" src="${product.dataImg}">
            <h4 class="product-name text-center pt-7 pb-5 font-semibold first-letter">${product.productsTitles}</h4>
            <p class="original-price pt-0 px-5 pb-5 text-center leading-5 line-through">${product.originalPrices}</p>
            <p class="original-price pt-0 px-5 pb-5 text-center leading-5">${product.offerPrices}</p>
            <div class="flex flex-col md:flex md:flex-row md:justify-center md:space-x-4">
                <a href="${product.url}" target="_blank">
                    <button class="add-to-cart-btn bg-blue-700 text-white font-sans duration-500 px-8 py-2 mx-auto hover:bg-pink-500 rounded-md flex mb-2">Buy at Iman's</button>
                </a>
    ...
    `;
      ...
      //   <button class="bg-orange-400 text-white font-sans duration-500 px-8 py-2 mx-auto hover:bg-pink-500 rounded-md flex mb-2">
      //     Buy at Shopee
      //   </button>
      // </a>;
    });
...
function purchaseProduct(ele) {
  ...
  if (ele.target.classList.contains("")) {
...
function findCartInfo() {
  ...
  let total = products.reduce((acc, product) => {
    let price = parseFloat(product.originalPrice.substr(2));
    // remove RM sign
    return (acc += price);
  }, 0);
  // });
...
function nameOfferPrice(productInfo) {
  ...
  return (
    "Name of Product: " + name + "%0aOffer Price: " + originalPrice + "%0a"
  );
}
...
function whatsApp() {
  ...
  products.forEach((product, index) => {
    ...
    totalPrice += parseFloat(originalPrice.substr(2)); // add the offerPrice to the totalPrice
  });
